...
export const Pagination = ({ itemsPerPage, showEntries, setShowEntries, data, currentPage, searchValues, setCurrentPage, theme }) => {
    ...
    return (
        <div className="pagination-container">
            <div style={{ display: "flex", gap: '1rem' }}>
                ...
                {itemsPerPage.length ?
                    <select id='rows-per-page' value={showEntries} onChange={(e) => {
            ...
            <div className="pagination">
                <button className="pagination-btn"
                    aria-label='previous'
                    style={{ background: theme?.primary?.light, borderColor: theme?.primary?.dark }}
                    disabled={currentPage === 0} onClick={() => setCurrentPage(page => page - 1)}>
                ...
                <button className="pagination-btn"
                    aria-label='next'
                    style={{ background: theme?.primary?.light, borderColor: theme?.primary?.dark }}
                    disabled={(currentPage + 1 >= Math.ceil(data.filter(searchValues).length / showEntries))}
